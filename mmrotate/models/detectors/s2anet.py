...
from .utils import AlignConvModule, CorrelativeAlignConvModule
...
class S2ANet(RotatedBaseDetector):
    ...
    def __init__(self,
        ...
        if self.align_conv_type == 'CorrelativeAlignConv':
            self.align_conv = CorrelativeAlignConvModule(self.feat_channels,
                                                         self.featmap_strides,
                                                         self.align_conv_size,
                                                         smooth_scale=align_cfgs['smooth_scale'])
    ...
    def forward_train(self,
        ...
        rois = self.fam_head.refine_bboxes(*outs)   # decode the bbox_preds
        # rois: list(indexed by images) of list(indexed by levels)
        align_feat = self.align_conv(x, rois)
        outs = self.odm_head(align_feat)
        loss_inputs = outs + (gt_bboxes, gt_labels, img_metas)
