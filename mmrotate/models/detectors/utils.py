...
class AlignConv(nn.Module):
    ...
    def get_offset(self, anchors, featmap_size, stride):
        ...
        cos, sin = torch.cos(a), torch.sin(a)    # aim     smooth a
...
class AlignConvModule(nn.Module):
    ...
    def forward(self, x, rbboxes):
        ...
        for x_scale, rbboxes_scale, ac_scale in zip(x, mlvl_rbboxes, self.ac):
            feat_refined_scale = ac_scale(x_scale, rbboxes_scale) # x--feature  rbboxes_scale--bbox_pred
            out.append(feat_refined_scale)
        return out


class CorrelativeAlignConv(nn.Module):
    '''
    ORIENTED CORRELATIVE FEATURE ALIGNMENT FOR REMOTE OBJECT DETECTION
    '''
    ...
    def __init__(self,
                 ...
                 stride=None,
                 deform_groups=1):
        super(CorrelativeAlignConv, self).__init__()
        self.pad_type_a = nn.ZeroPad2d(padding=(1, 1, 1, 1))#nn.ReplicationPad2d(padding=(1, 1, 1, 1))
        self.smooth_kernel=torch.ones(1,1,smooth_scale,smooth_scale)/(smooth_scale*smooth_scale)
        self.kernel_size = kernel_size
        self.stride = stride
        self.deform_conv = DeformConv2d(
            in_channels,
    ...
    def get_offset(self, anchors, featmap_size, stride):
        ...
        a=a.reshape(1,1,featmap_size[0],featmap_size[1])
        a=self.pad_type_a(a)
        a=F.conv2d(a, self.smooth_kernel.to(a.device),stride=1)
        a=torch.flatten(a)
...
class CorrelativeAlignConvModule(nn.Module):
    """ORIENTED CORRELATIVE FEATURE ALIGNMENT FOR REMOTE OBJECT DETECTION
    ...
    def __init__(self, in_channels, featmap_strides, align_conv_size,smooth_scale=3):
        super(CorrelativeAlignConvModule, self).__init__()
        self.in_channels = in_channels
        self.featmap_strides = featmap_strides
        self.align_conv_size = align_conv_size
